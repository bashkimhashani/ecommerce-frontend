  ...
  <section class="rounded-md border border-slate-200 bg-white">
    <div class="border-b border-slate-200 px-4 py-3">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 class="text-sm font-semibold text-slate-950">AI Insights</h2>
          <p class="mt-1 text-xs text-slate-500">AI-generated estimate based on your store data</p>
        </div>
        <p v-if="generatedAt" class="text-xs font-medium text-slate-500">{{ generatedAt }}</p>
      </div>
    </div>
    ...
    <div class="p-4">
      ...
      <div v-else-if="report" class="space-y-3">
        <p class="whitespace-pre-line text-sm leading-6 text-slate-700">{{ report.content }}</p>
      ...
      <div v-else class="py-6 text-center">
        <p class="text-sm font-semibold text-slate-700">No AI report yet.</p>
        <p class="mt-1 text-sm text-slate-500">The latest nightly report will appear here after generation.</p>
      </div>
    </div>
  </section>
</template>
