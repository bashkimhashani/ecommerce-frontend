      ...
      >
        <header class="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-800">
          <div class="flex items-center gap-3">
            ...
            >
            <div>
              <h2 class="text-sm font-semibold text-slate-950 dark:text-white">Shopping assistant</h2>
              <p class="text-xs text-slate-500 dark:text-slate-400">Helper bot for this tech store</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div ref="messageList" class="flex-1 space-y-4 overflow-y-auto bg-slate-50 px-4 py-4 dark:bg-slate-900">
          <div v-if="chatStore.messages.length === 0" class="rounded border border-slate-200 bg-white p-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300">
          ...
          >
            ...
            >
              <img v-if="userAvatar" class="h-full w-full object-cover" :src="mediaUrl(userAvatar)" alt="">
              <span v-else>{{ userInitials }}</span>
            </div>

            <div class="flex min-w-0 max-w-[85%] flex-col" :class="message.role === 'user' ? 'items-end' : 'items-start'">
              <p
          ...
          <div v-if="chatStore.isLoading" class="text-xs text-slate-500 dark:text-slate-400">Thinking...</div>
        </div>

        <form class="border-t border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950" @submit.prevent="sendMessage">
