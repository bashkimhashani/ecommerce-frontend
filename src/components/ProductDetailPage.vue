...
<template>
  <section class="min-w-0 flex-1 bg-transparent px-5 py-5 text-slate-950 dark:text-slate-100 sm:px-6">
    <button
      type="button"
    ...
    <div v-else-if="product" class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      <div class="min-w-0">
        <div class="overflow-hidden rounded-2xl border border-cyan-100 bg-gradient-to-br from-cyan-50 via-slate-100 to-emerald-50 shadow-xl shadow-cyan-950/10 dark:border-cyan-400/10 dark:from-slate-900 dark:via-slate-950 dark:to-cyan-950">
          <img
            v-if="selectedImage"
          ...
          >
          <div v-else class="flex aspect-[4/3] items-center justify-center px-6 text-center text-sm font-bold text-slate-400 dark:text-slate-300">
            {{ product.name }}
          </div>
        </div>
        ...
        <div v-if="galleryImages.length > 1" class="mt-3 grid grid-cols-4 gap-3 sm:grid-cols-6">
          <button
            ...
            :class="index === selectedImageIndex ? 'border-cyan-500 ring-2 ring-cyan-400 dark:border-cyan-300 dark:ring-cyan-300' : 'border-slate-200 hover:border-cyan-300 dark:border-slate-700 dark:hover:border-cyan-400/40'"
        ...
        <div class="mt-6 rounded-2xl border border-cyan-100 bg-white shadow-lg shadow-cyan-950/5 dark:border-cyan-400/10 dark:bg-slate-900">
          <div class="flex border-b border-cyan-100 dark:border-cyan-400/10">
            <button
              type="button"
              class="px-4 py-3 text-sm font-semibold"
              :class="activeTab === 'specs' ? 'border-b-2 border-cyan-500 text-cyan-700 dark:border-cyan-300 dark:text-cyan-100' : 'text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-100'"
            ...
            <button
              type="button"
              class="px-4 py-3 text-sm font-semibold"
              :class="activeTab === 'variants' ? 'border-b-2 border-cyan-500 text-cyan-700 dark:border-cyan-300 dark:text-cyan-100' : 'text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-100'"
      ...
      <aside class="h-fit rounded-2xl border border-cyan-100 bg-white p-5 shadow-xl shadow-cyan-950/10 dark:border-cyan-400/10 dark:bg-slate-900">
        ...
        <div class="mt-5 flex items-end justify-between gap-4">
          ...
          <p class="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-200">
            {{ totalStock }} in stock
          </p>
        </div>
        ...
        <p v-if="addToCartStatus" class="mt-3 rounded-xl bg-emerald-50 px-3 py-2 text-sm font-bold text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-200">
          {{ addToCartStatus }}
        </p>
      </aside>
    </div>
