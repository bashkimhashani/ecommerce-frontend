...
<template>
  ...
  >
    ...
    <div v-if="isLoading" class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      <div class="h-[650px] animate-pulse rounded-2xl bg-cyan-50 dark:bg-slate-900"></div>
    ...
    <div v-else-if="product" class="space-y-6">
      <div class="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div class="min-w-0">
          <div
            class="min-h-[650px] overflow-hidden rounded-2xl border border-cyan-100 bg-gradient-to-br from-cyan-50 via-slate-100 to-emerald-50 shadow-xl shadow-cyan-950/10 dark:border-cyan-400/10 dark:from-slate-900 dark:via-slate-950 dark:to-cyan-950"
          >
            <img
              v-if="selectedImage"
              :src="selectedImage.src"
              :alt="selectedImage.alt_text || product.name"
              class="h-[650px] w-full bg-white object-contain p-4 dark:bg-slate-900"
            />

            <div
              v-else
              class="flex h-[650px] items-center justify-center px-6 text-center text-sm font-bold text-slate-400 dark:text-slate-300"
            >
              {{ product.name }}
            </div>
          </div>
        ...
        <aside
          class="flex min-h-[650px] flex-col rounded-2xl border border-cyan-100 bg-white p-5 shadow-xl shadow-cyan-950/10 dark:border-cyan-400/10 dark:bg-slate-900"
        >
      ...
      <div
        class="w-full rounded-2xl border border-cyan-100 bg-white shadow-lg shadow-cyan-950/5 dark:border-cyan-400/10 dark:bg-slate-900"
      >
        ...
        <div class="p-4">
          <dl
            v-if="activeTab === 'specs' && specs.length"
          ...
          <div v-else-if="variants.length" class="divide-y divide-slate-200 dark:divide-slate-700">
            <label
              ...
              :key="variant.id"
              class="grid cursor-pointer gap-3 py-3 text-sm sm:grid-cols-[32px_repeat(5,minmax(0,1fr))]"
            ...
            >
              ...
              <span class="font-medium text-emerald-600 dark:text-emerald-300">
                ${{ Number(variant.variant_price || product.price).toFixed(2) }}
              </span>
            </label>
          </div>
